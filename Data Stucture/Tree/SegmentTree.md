...
template<typename _Tp>
struct Node{
    std::unique_ptr<_Tp> v;
    std::shared_ptr<Node<_Tp>> lc, rc;
    template<class ...args>
    Node(const args&... _v):lc(nullptr), rc(nullptr) { v = std::unique_ptr<_Tp>(new _Tp(_v...)); }
};
template<typename _Tp>
struct Seg {
    template<class ...args>
    void create(std::shared_ptr<Node<_Tp>> &x, const args&...v);
    template<class ...args>
    void apply(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const args&...v);
    template<typename T>
    T merge(const T &x, const T &y);
    template<typename T, class ...args>
    T ret(std::shared_ptr<Node<_Tp>> &x, const args& ...v);
    template<class ...args>
    void poll(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const args&...v);
    template<class ...args>
    void push(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const args&...v);
    template<class ...args> 
    void init(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const args &...v, void(*init_apply)(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const args &...v)) {
        create(x);
        if (l == r) {
            init_apply(x, l, r, v...);
            return;
    ...
    template<class ...args>
    void upd(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const int &L, const int &R, const args &...v) {
        create(x);
        if (l > R || r < L) return;
        if (L <= l && r <= R) {
            apply(x, l, r, v...);
    ...
    template<typename T, class ...args>
    T que(std::shared_ptr<Node<_Tp>> &x, const int &l, const int &r, const int &L, const int &R, const args& ...v) {
        create(x);
        if (l > R || r < L) return T(0);
        if (L <= l && r <= R) return ret<T>(x, v...);
        push(x, l, r);
    ...
    Seg() {
        rt = std::shared_ptr<Node<_Tp>>(new Node<_Tp>());
    }
    std::shared_ptr<Node<_Tp>> rt;
};
const int mod = 998244353;
const int N = 5 + 5e5;
int a[N], inv[N];
...
template<> template<>
void Seg<node>::create(std::shared_ptr<Node<node>> &x) {
    if (x == nullptr) x = std::shared_ptr<Node<node>>(new Node<node>());
}
template<> template<>
void Seg<node>::apply(std::shared_ptr<Node<node>> &x, const int &l, const int &r, const int &pre, const int &id) {
    int g = mul(inv[pre], a[id + 1]);
    x->v->l = add(x->v->l, mul(x->v->r, id));
    x->v->r = mul(x->v->r, g);
    x->v->l = sub(x->v->l, mul(x->v->r, id));
...
template<> template<>
void Seg<node>::poll(std::shared_ptr<Node<node>> &x, const int &l, const int &r) {
    create(x->lc); create(x->rc);
    x->v->l = add(x->lc->v->l, x->rc->v->l);
    x->v->r = add(x->lc->v->r, x->rc->v->r);
}
template<> template<>
void Seg<node>::push(std::shared_ptr<Node<node>> &x, const int &l, const int &r) {
    create(x->lc); create(x->rc);
    x->lc->v->l = add(x->lc->v->l, mul(x->v->lt, x->lc->v->r));
    x->lc->v->r = mul(x->lc->v->r, x->v->rt);
    x->rc->v->l = add(x->rc->v->l, mul(x->v->lt, x->rc->v->r));
...
template<> template<>
int Seg<node>::ret<int>(std::shared_ptr<Node<node>> &x, const int &id) {
    return add(x->v->l, mul(x->v->r, id));
}
template<> template<>
int Seg<node>::merge<int>(const int &x, const int &y) {
...
void init_apply(std::shared_ptr<Node<node>> &x, const int &l, const int &r) {
    x->v->l = -mul(l - 1, mul(a[l], a[l]));
    x->v->r = mul(a[l], a[l]);
}
```
